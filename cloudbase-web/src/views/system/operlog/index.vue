    ...
    <el-card>
      ...
      >
        ...
        <el-table-column prop="costTime" label="耗时(ms)" width="90" align="right">
          <template #default="{ row }">
            <span :style="{ color: row.costTime > 3000 ? '#f56c6c' : '' }">{{ row.costTime }}</span>
          </template>
        </el-table-column>
        <el-table-column label="状态" width="80" align="center">
          <template #default="{ row }">
            <el-tag :type="row.success === 1 || row.status === 1 ? 'success' : 'danger'" size="small">
              {{ (row.success === 1 || row.status === 1) ? '成功' : '失败' }}
            </el-tag>
          </template>
        </el-table-column>
...
const operTypeTag = (type: string) => ({
  INSERT: 'primary', UPDATE: 'warning', DELETE: 'danger', QUERY: 'info', EXPORT: 'success', GRANT: 'success', OTHER: 'info'
}[type] || 'info')
...
function handleExport() {
  const exportData = tableData.value.map((row: any) => ({
    ...
    '操作类型': row.operType,
    '请求方法': row.method || '',
    '操作人': row.operUserName,
    'IP地址': row.operIp,
    '耗时(ms)': row.costTime,
    '状态': (row.success === 1 || row.status === 1) ? '成功' : '失败',
    '操作时间': row.operTime
  }))
  ...
  ws['!cols'] = [
    { wch: 15 }, { wch: 12 }, { wch: 30 }, { wch: 12 }, { wch: 18 }, { wch: 10 }, { wch: 8 }, { wch: 20 }
  ]
